...
@dag(
    ...
    tags=["forecast", "monitor", "daily"],
)
def dag_forecast():

    run_forecast = HttpOperator(
        task_id="run_forecast",
        http_conn_id="manager-server-api",
        endpoint="/run/prophet?retrain=scheduled",
        method="POST",
    ...
    poll_forecast = HttpSensor(
        task_id="poll_forecast",
        http_conn_id="manager-server-api",
        endpoint="/status/{{ ti.xcom_pull(task_ids='run_forecast')['job_id'] }}",
