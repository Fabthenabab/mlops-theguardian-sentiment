...
dag_monitor
├── Task 1 : POST /run/monitor?mode=compare  → xcom_push(job_id)
├── Task 2 : GET  /status/{job_id}           → poll jusqu'à "done"
└── Task 3 : lit theguardian.monitor → drift=True ?
              → POST /run/prophet?retrain=evidently_drift
              → GET  /status/{job_id} → poll jusqu'à "done"

dag_snapshot (hebdomadaire)
├── Task 1 : POST /run/monitor?mode=snapshot → xcom_push(job_id)
└── Task 2 : GET  /status/{job_id}           → poll jusqu'à "done"
```
...
Pour tester le pipeline de détection de drift et de réentraînement sans attendre une dérive naturelle des données, il faut pouvoir injecter un drift artificiel dans la base.

## Décision : injection via articles fictifs préfixés drift_

Les articles fictifs sont insérés dans theguardian.articles avec un id préfixé drift_<incrément> et des valeurs extrêmes :
- sentiment_label : positive
- sentiment_score : 1.0
- date : depuis la dernière date connue en base

Ces valeurs créent un biais statistique détectable par Evidently sur la distribution de sentiment_label et sentiment_score.
...
— il ne simule pas un drift subtil de production mais suffit à valider le pipeline de détection et de réentraînement.


---
# ADR-013 — Monitoring de drift avec Evidently
Date : 2026-05-08
## Contexte

Le système produit des scores de sentiment FinBERT sur des articles Guardian business. La distribution de ces scores peut dériver dans le temps. Un drift détecté doit déclencher un réentraînement de Prophet, mais pas de FinBERT : Si le contexte a changé, il est naturel que le sentiment général reflété dans la presse change, néanmoins le modèle de projections Prophet doit être actualisé pour s'adapter au changement de contexte. 

## Ce que surveille Evidently

- Distribution de sentiment_label (positive / negative / neutral)
- Distribution de sentiment_score (niveau de certitude de FinBERT)

## Architecture : worker_monitor.py

Evidently vit dans un worker ponctuel — pas dans un DAG Airflow.
Airflow orchestre, le worker décide.
Le worker écrit ses conclusions dans une table de monitoring, consultable par les DAGs d'Airflow.

Deux modes via argparse :

  --mode snapshot
    Sauvegarde la distribution actuelle comme référence dans S3
    Format : parquet (plus compact que CSV)
    Schedule : hebdomadaire (dag_snapshot)

  --mode compare
    Charge la référence depuis S3
    Compare avec la distribution actuelle via Evidently
    Écrit le résultat dans theguardian.monitor
    Schedule : quotidien (dag_monitor)

## Seuils de drift

Définis comme variables d'environnement — configurables sans rebuild :

  DRIFT_THRESHOLD_LABEL=0.10   (distribution sentiment_label)
  DRIFT_THRESHOLD_SCORE=0.10   (distribution sentiment_score)

Seuils volontairement sensibles en contexte POC.
À recalibrer en production sur données réelles.

## Table theguardian.monitor

```sql
CREATE TABLE theguardian.monitor (
    id          SERIAL PRIMARY KEY,
    run_id      VARCHAR(36),
    run_date    DATE,
    mode        VARCHAR(10),      -- snapshot | compare
    drift       BOOLEAN,          -- drift détecté oui/non
    drift_score FLOAT,            -- score de drift Evidently
...
Chemin : s3://{bucket}/monitor/reference.parquet
Un seul fichier de référence — écrasé à chaque snapshot.
Le snapshot hebdomadaire glisse sur la fenêtre la plus récente.

## Décision Airflow

dag_monitor lit theguardian.monitor après le run compare.
Si drift=True → POST /run/prophet?retrain=evidently_drift.
Airflow ne connaît pas Evidently — il lit un booléen en base.
